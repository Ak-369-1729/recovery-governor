...
# =============================================================================
# PHASE-3 SCHEMAS: PREDICT, PREVENT, NETWORK HEALTH, PREDICTION EVALUATION
# =============================================================================
...
class FailurePrediction(BaseModel):
    ...
    candidate_preventive_actions: List[ActionType] = Field(default_factory=list)
    prediction_source: str = "SYNTHETIC_PREDICTIVE_MODEL"
    model_version: str = "v3.1-simulation"
    timestamp: str

class PreventiveERVCalculation(BaseModel):
    action: ActionType
    simulated_failure_probability: float
    success_probability_with_action: float
    payment_amount: float
    prevented_gross_value: float
    intervention_cost: float
    risk_cost: float
    friction_cost: float
    net_preventive_erv: float
    is_viable: bool
    formula_breakdown: str

class PreventiveGovernorDecision(BaseModel):
    decision_id: str
    payment_id: str
    failure_prediction: FailurePrediction
    governor_status: GateStatus  # PASSED or BLOCKED
    decision_outcome: str  # APPROVED, REJECTED, SUPPRESSED
    selected_action: ActionType
    net_preventive_erv: float
    erv_breakdown: Dict[str, PreventiveERVCalculation]
    policy_checks: List[PolicyGateCheck]
    blocked_actions: List[str]
    explainability: Dict[str, str]  # why_act, why_this_action, why_not_alternatives, safety_gates_summary
    timestamp: str

class RailHealthTelemetry(BaseModel):
    rail_id: str
    rail_name: str
    ...
    simulation_disclaimer: str = (
        "SIMULATED NETWORK HEALTH: Generated by deterministic scenario simulation; not live bank telemetry."
    )

class MerchantPolicyConfig(BaseModel):
    merchant_id: str
    allow_prevention: bool = True
    max_prevention_friction: float = 20.0
    prohibited_preventive_actions: List[ActionType] = Field(default_factory=list)
    preferred_payment_methods: List[PaymentMethod] = Field(default_factory=list)
    max_daily_preventions: int = 50
    override_global_safety: bool = False  # Note: Ignored by Governor invariant

class PredictionOutcomeEvaluation(BaseModel):
    prediction_id: str
    payment_id: str
    predicted_probability: float
    predicted_failure: bool
    predicted_failure_type: Optional[str] = None
    actual_outcome: str  # "SUCCESS" or "FAILED"
    actual_failure: bool
    actual_failure_type: Optional[str] = None
    probability_error: float
    brier_contribution: float
    classification_result: PredictionClassification
    prediction_source: str = "SYNTHETIC_PREDICTIVE_MODEL"
    timestamp: str

class ReliabilityBucket(BaseModel):
    range_label: str
    predicted_average: Any  # float or "N/A"
    actual_failure_rate: Any  # float or "N/A"
    sample_count: int
    prediction_error: Any  # float or "N/A"

class PredictionReliabilityMetrics(BaseModel):
    total_predictions: int
    true_positives: int
    true_negatives: int
    false_positives: int
    false_negatives: int
    precision: Any  # float or "N/A"
    recall: Any  # float or "N/A"
    f1_score: Any  # float or "N/A"
    false_positive_rate: Any  # float or "N/A"
    accuracy: Any  # float or "N/A"
    brier_score: Any  # float or "N/A"
    reliability_buckets: List[ReliabilityBucket]
    evaluation_environment: str = "SYNTHETIC_GROUND_TRUTH"

class PreventionEconomicsMetrics(BaseModel):
    total_payment_attempts: int
    high_risk_predictions: int
    preventive_candidates: int
    preventive_actions_approved: int
    preventive_actions_rejected: int
    failures_predicted: int
    failures_actually_observed: int
    failures_prevented: int
    false_positive_interventions: int
    unnecessary_interventions: int
    preventive_intervention_cost: float
    estimated_prevented_gmv: float
    net_preventive_economic_value: float

class UnifiedLifecycleTrace(BaseModel):
    lifecycle_id: str
    payment_id: str
    ...
    preventive_execution: Optional[Dict[str, Any]] = None
    ...
    prediction_evaluation: Optional[PredictionOutcomeEvaluation] = None
